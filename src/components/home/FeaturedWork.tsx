...
export function FeaturedWork({ language }: FeaturedWorkProps) {
    ...
    const filteredWorks = selectedCategory === "All" || selectedCategory === "అన్నీ"
        ? featuredWorks
            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                <ScrollReveal direction="up" delay={0.2} duration={0.6}>
                    <div className="flex flex-wrap justify-center gap-3 mb-12">
                        {t.categories.map((category, index) => (
                            <Button
                                key={index}
                                variant={selectedCategory === category || (selectedCategory === "All" && category === "అన్నీ") ? "gold" : "outline"}
                                size="sm"
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 max-w-6xl mx-auto mb-8">
                    {filteredWorks.map((work, index) => (
                        <ScrollReveal key={work.id} direction="up" delay={index * 0.1} duration={0.5} width="100%">
                            <Card
